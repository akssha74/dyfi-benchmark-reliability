...
and single-use authorization of the decision-bearing protected evaluation.

On the frozen M ≥ 5 snapshot (7,520 source records; 2,362 eligible events;
430-event temporal holdout), sequence grouping produced a Brier score 0.006
lower and an AUROC 0.012 higher than random-event splitting. We report this
descriptive point estimate as a
...
generated from frozen analysis outputs and verified by the released automated
validation. After the token-authorized result was fixed, two
token-free verification replays reproduced the recorded results and holdout
predictions exactly.
...
reproduced the 16-page manuscript and passed the released manuscript validator
and benchmark test suite in a newly created pinned environment. The authors
received no specific funding.

The same tagged release contains a non-peer-reviewed manuscript PDF and source;
we disclose this as a preprint under Springer Nature's preprint policy. The
preprint has no DOI. No separate licence is granted for the manuscript text;
copyright remains with the authors pending publication (the released software
and data retain their stated MIT and CC0 terms). If published, we will update
the public record with the article DOI and journal URL.
